...
"""Download model checkpoints from the s3.ereuna.org object store to models/.

Usage:
    python3.11 scripts/download_models_from_s3.py
    python3.11 scripts/download_models_from_s3.py --dry-run
    python3.11 scripts/download_models_from_s3.py --dest models --bucket nyx --prefix models/
    python3.11 scripts/download_models_from_s3.py --include "*.pth" --include "*.safetensors"
...
def main() -> int:
    ...
    parser.add_argument("--dest", default="models", help="Local directory to download into (default: models)")
    ...
    parser.add_argument(
        ...
        help="fnmatch pattern (relative to --prefix) to include; repeatable. Default: * (everything)",
    )
    parser.add_argument(
    ...
    total_gb = sum(o["size"] for o in objects) / (1024 ** 3)
    s3.logger.info(
        "Found %d object(s) (%.1f GB) under s3://%s/%s matching %s (excluding dirs: %s)",
        len(objects), total_gb, args.bucket, args.prefix, include_globs, sorted(exclude_dirs),
    )
