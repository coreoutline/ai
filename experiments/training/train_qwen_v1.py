...
with open("/root/transformer/data/content.txt", "r", encoding="utf-8") as f:
    raw_text = f.read()
    raw_text = raw_text
...
def train_model_simple(model, train_loader, val_loader, tokenizer=tokenizer):
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, target_batch in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
            ...
            torch.save(model.state_dict(), f"/root/transformer/models/chaos_2.pth")


            model.eval()
            start_context = "Churn rate"
...
core_model.load_state_dict(torch.load("/root/transformer/models/core_model_weights.pth", map_location="cpu"))
...
train_model_simple(core_model, train_loader, val_loader, tokenizer=tokenizer)